...
- 将所有 `/api/*` 请求代理到后端服务器 `http://localhost:8081`
- 解决开发环境的跨域问题
...
1. **智能环境检测**
   ...
   - 开发环境：使用相对路径（通过 `buildApiUrl` 统一拼接），依赖 Vite 代理到 8081
   - 生产环境：默认 `http://localhost:8081`，可通过运行时注入 `window.API_BASE` 覆盖
...
↓ (Vite 代理)
后端请求: http://localhost:8081/api/news/articles
```

### 生产环境

```text
前端请求: http://localhost:8081/api/news/articles
↓ (直接请求)
后端请求: http://localhost:8081/api/news/articles
```
...
1. 确保 `vite.config.ts` 中的代理目标与后端服务器地址一致（当前使用 8081）
