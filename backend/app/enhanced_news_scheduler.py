    ...
    async def _crawl_articles_batch(
        ...
        for i in range(0, len(search_results), self.crawl_batch_size):
            ...
            if batch_urls:
                ...
                batch_results = await crawler.batch_crawl_articles(batch_urls)
                ...
                for search_result, crawl_result in zip(batch, batch_results):
                    if crawl_result.get('status') == 'success':
                        ...
                        logging.warning(f"Failed to crawl {crawl_result.get('url', 'unknown')}: {crawl_result.get('error', 'unknown error')}")
        ...
        try:
            ...
            if article_id:
                ...
                await self.storage.save_duplicate_detection(
                    url=article_data.get('url', ''),
                    content_hash=self.deduplicator._generate_content_hash(article_data.get('content', '')),
                    fingerprint=self.deduplicator._generate_fingerprint(article_data.get('title', ''), article_data.get('content', ''))
                )
