...
from ..core.models import (
    ...
    NewsCategory, SentimentType, Watchlist, NewsQueryTemplate
)
...
class NewsSearchService:
    ...
    def _extract_published_datetime(self, result: Dict[str, Any]) -> Optional[datetime]:
        ...
        Tries common fields: 'published', 'publishedDate', 'date', 'published_at', 'published_ts'.
        Accepts ISO-8601 strings or epoch seconds.
        Returns timezone-naive UTC-like datetime when possible, else None.
        """
    ...
    async def search_stock_news(self, symbol: str, company_name: str = None) -> List[Dict[str, Any]]:
        """
        针对单个股票进行新闻搜索，带“符号失败时改用名称/公司名”的自动回退：

        规则：
        - 先尝试以股票代码为主的查询（如“symbol 股票 财经”、“symbol”）；
        - 若符号检索结果偏少（< 3）或请求异常，则“去掉股票代号”，改用“公司名/股票名”进行更宽泛主题检索；
          主题包含：公司/经营/业务/公告/舆情/产品/招聘/投资者关系 等，不再强制偏向财经；
        - 若依旧稀少，则扩大时间窗至 month；仍不足时放开语言限制（language=None）。
        - 始终对 URL 去重并限制返回总量。
        """

        async def _run_queries(qs: List[str], time_range: str, language: Optional[str]) -> List[Dict[str, Any]]:
            out: List[Dict[str, Any]] = []
            for q in qs:
                try:
                    rs = await self.search_news(
                        query=q,
                        category="general",
                        time_range=time_range,
                        max_results=15,
        ...
        # 0) 从模板范式加载查询（symbol/global），按 priority 降序
        tmpl_qs: List[str] = []
        try:
            from ..core.db import SessionLocal as _SL
            _s = _SL()
            try:
                q = select(NewsQueryTemplate).where(NewsQueryTemplate.enabled == True).order_by(NewsQueryTemplate.priority.desc())
                rows = _s.execute(q).scalars().all()
                for r in rows:
                    if r.scope == 'symbol' and (r.target or '').upper() != symbol.upper():
                        continue
                    if r.scope == 'industry':
                        # 暂未接入行业上下文，这里先跳过；未来可传入 industry 参数
                        continue
                    try:
                        t = (r.template or '').format(symbol=symbol, name=(company_name or ''), industry='')
                        if t and t not in tmpl_qs:
                            tmpl_qs.append(t)
                    except Exception:
                        continue
            finally:
        ...
        # 1) 符号优先（内置默认）
        symbol_qs: List[str] = [f"{symbol} 股票 财经", f"{symbol} 公告", symbol]
        lang = os.getenv("SEARXNG_LANGUAGE", "zh-CN")
        all_results: List[Dict[str, Any]] = []
        pref_qs = (tmpl_qs[:5] if tmpl_qs else []) + symbol_qs
        sym_results = await _run_queries(pref_qs, time_range="week", language=lang)
        all_results.extend(sym_results)

        # 2) 若符号结果不足，则切换为“名称/公司名”更宽泛的主题组合（不强制财经）
        need_name_fallback = len(sym_results) < 3
        if company_name and company_name.strip():
            ...
            # 模板可能也包含 name 相关的查询，附加到 name 阶段的前面
            if tmpl_qs:
                for t in tmpl_qs[:5]:
                    if '{name}' in (t or '') or (company_name and company_name in t):
                        if t not in name_qs:
                            name_qs.insert(0, t)
            if need_name_fallback:
                name_rs = await _run_queries(name_qs, time_range="week", language=lang)
                all_results.extend(name_rs)

                # 3) 仍不足则扩大时间窗
                if len(all_results) < 6:
                    name_rs_m = await _run_queries(name_qs, time_range="month", language=lang)
                    all_results.extend(name_rs_m)

                # 4) 最后放开语言限制
                if len(all_results) < 3:
                    name_rs_any = await _run_queries(name_qs, time_range="month", language=None)
                    all_results.extend(name_rs_any)
        else:
            # 没有公司名可用，则用符号策略扩大检索范围
            if len(all_results) < 6:
                sym_more = await _run_queries(symbol_qs, time_range="month", language=lang)
                all_results.extend(sym_more)
            if len(all_results) < 3:
                sym_any = await _run_queries(symbol_qs, time_range="month", language=None)
                all_results.extend(sym_any)
...
class NewsProcessor:
    def __init__(self):
        ...
        # Per-request HTTP timeout (seconds) for httpx clients
        try:
            self._http_timeout = float(os.getenv("NEWS_HTTP_TIMEOUT", "30"))
        except Exception:
    ...
    async def _extract_content(self, url: str, soup: Optional[BeautifulSoup] = None) -> Optional[str]:
        ...
        try:
            ...
            try:
                content = self._maybe_fix_mojibake(content)
            except Exception:
                pass
