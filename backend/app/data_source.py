...
def fetch_daily_akshare(symbol: str, start_date: str = None) -> pd.DataFrame:
    ...
    # akshare 要求 start_date 为 YYYYMMDD 字符串或 None
...
def fetch_daily(symbol: str, start_date: str | None = None) -> pd.DataFrame:
    ...
    if DATA_SOURCE == "tushare" and TUSHARE_TOKEN:
        try:
            ...
            # 若 tushare 不可用（如权限不足等），则回退 akshare
            print(f"[fetch_daily] Tushare failed for {symbol}: {e}. Falling back to akshare…")
            return fetch_daily_akshare(symbol, start_date)
    # 默认使用 akshare
    return fetch_daily_akshare(symbol, start_date)


def _load_fund_flow_rank_today(max_age_sec: int = 20, retries: int = 3) -> pd.DataFrame:
    """获取并缓存“今日资金流排行”DataFrame。

    - max_age_sec: 允许的缓存时长，默认 20 秒，避免短时间内重复打点。
    - retries: 最大重试次数。失败后会按指数回退等待。

    若所有重试均失败且缓存中仍有近 5 分钟内的数据，则返回缓存数据；否则抛出最后一次异常。
    """
    now = time.time()
    # 先尝试使用缓存
    ...
    for attempt in range(retries):
        try:
            ...
            with _fund_flow_rank_lock:
                _fund_flow_rank_cache["records"] = records
                _fund_flow_rank_cache["ts"] = time.time()
            return rdf
        except Exception as e2:
            alt = _fetch_fund_flow_intraday_eastmoney(sym)
            if alt:
                alt_df = pd.DataFrame([alt], columns=cols)
                return alt_df
            _log_throttled(
                f"fund_flow_fallback_error:{sym}",
                logging.ERROR,
                f"Error in fund flow fallback for {sym}: {e2}",
                120,
            )
    ...
    if last_err is not None:
        raise last_err
    raise RuntimeError("fund flow rank fallback unavailable")
...
def fetch_fund_flow_daily(symbol: str, start_date: str | None = None, include_today_rank: bool = False) -> pd.DataFrame:
    ...
    try:
        ...
        logger.warning(f"akshare.stock_individual_fund_flow for {sym} failed, will try fallback. Error: {e}")
    ...
    # 主接口失败或返回空，执行回退策略
    # 注意：include_today_rank 参数决定是否执行此回退。EOD任务应设为False。
    if not include_today_rank:
        logger.warning(f"Main fund flow interface failed for {sym} and fallback is disabled.")
        return pd.DataFrame(columns=cols)
        
    logger.info(f"Fallback for {sym}: trying stock_individual_fund_flow_rank.")
    try:
        # 回退接口：统一通过缓存+重试层获取，避免对目标接口频繁断连
        ...
        code_col = "代码" if "代码" in rdf.columns else "symbol"
        ...
        out = {
            "symbol": sym,
            "trade_date": datetime.now(gettz("Asia/Taipei")).date(),
            # Rank 接口亦通常以“万元”为单位，统一转换为“元”
            "main_net": pd.to_numeric(pick(["今日主力净流入-净额","主力净流入-净额"]), errors="coerce") * 1e4,
            "main_ratio": pd.to_numeric(pick(["今日主力净流入-净占比","主力净流入-净占比"]), errors="coerce"),
            "super_net": pd.to_numeric(pick(["今日超大单净流入-净额","超大单净流入-净额"]), errors="coerce") * 1e4,
            "super_ratio": pd.to_numeric(pick(["今日超大单净流入-净占比","超大单净流入-净占比"]), errors="coerce"),
            "large_net": pd.to_numeric(pick(["今日大单净流入-净额","大单净流入-净额"]), errors="coerce") * 1e4,
            "large_ratio": pd.to_numeric(pick(["今日大单净流入-净占比","大单净流入-净占比"]), errors="coerce"),
            "medium_net": pd.to_numeric(pick(["今日中单净流入-净额","中单净流入-净额"]), errors="coerce") * 1e4,
            "medium_ratio": pd.to_numeric(pick(["今日中单净流入-净占比","中单净流入-净占比"]), errors="coerce"),
            "small_net": pd.to_numeric(pick(["今日小单净流入-净额","小单净流入-净额"]), errors="coerce") * 1e4,
        ...
        if alt:
            ...
            for field in numeric_fields:
                alt[field] = pd.to_numeric(alt.get(field), errors="coerce")
...
def get_spot_snapshot(symbols: list[str]):
    ...
    try:
        from .db import get_redis_client  # local import to avoid hard dependency at import time
    ...
    try:
        ...
        try:
            from .db import engine as _engine2
            for s in symbols:
                sym = normalize_symbol(s)
                if sym in out:
                    continue
        ...
        try:
            ...
            # 确保所有请求的标的都能返回：优先粘性缓存，最后回退到数据库日线
            from .db import engine as _engine3
            for s in symbols:
                sym = normalize_symbol(s)
                if sym in out:
                    continue
