...
# 后端端口 8081；SearXNG 若未在 .env 中指定则使用默认 10000；LLM 优先使用 Azure OpenAI 配置。
...
API_BASE = os.getenv("API_BASE", "http://localhost:8081").rstrip('/')
BACKEND_BASE_URL = os.getenv("BACKEND_BASE_URL", API_BASE)  # 兼容 enhanced_news_fetch 模块
MOVERS_API_URL = os.getenv("MOVERS_API_URL", "http://localhost:8081/api/movers/live_insight?limit=20")
...
def discover_movers_url(candidates: Optional[List[str]] = None) -> str:
    if candidates is None:
        candidates = [
            MOVERS_API_URL,
            "http://localhost:8081/api/movers/live_insight?limit=20",
            "http://127.0.0.1:8081/api/movers/live_insight?limit=20",
    ...
    for url in candidates:
        try:
            ...
            continue
    raise RuntimeError("无法发现可用的 movers API，请确认后端服务已在 8081 或指定端口运行。")
